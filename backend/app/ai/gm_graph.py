...
from backend.app.ai.prompts.gm_turn_prompts import GM_TURN_PROMPT
from backend.app.domain.action import PlayerAction
from backend.app.domain.context import TurnHistory, TurnResult, GMTurnOutput
...
# """.strip()
#
# #将GMState转换成ChatPromptTemplate需要的字典
# def build_template_prompt_input(state: GMState) -> dict[str, str]:
#     scene = state.get("scene", {})
#
#     return {
#         "world": state.get("world", "").strip(),
#         "player_actions": _format_actions(
#             state.get("player_actions", [])
#         ),
#         "host_note": state.get("host_note", "").strip() or "无。",
...
#     }
#
# @deprecated("用 GM_TURN_PROMPT 替代")
...
def call_llm(state: GMState) -> dict[str, Any]:
    ...
    chain = GM_TURN_PROMPT | structured_llm
    result: GMTurnOutput = chain.invoke(
        build_template_prompt_input(state),
    )
